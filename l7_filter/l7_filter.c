...
__declspec(dllexport) int check_l7_protocol(const uint8_t* data, int data_len) {
    if (data == NULL || data_len == 0) return 0; // no_data
    ...
    if (data_len >= 7 || data_len >= 4) {
        if ((data[0] == 'G' && data[1] == 'E' && data[2] == 'T' && data[3] == ' ') ||
            (data[0] == 'P' && data[1] == 'O' && data[2] == 'S' && data[3] == 'T')) {
            return 2; // http GET/POST
        }
        if (data[0] == 'H' && data[1] == 'T' && data[2] == 'T' && data[3] == 'P' && data[4] == '/' && data[5] == '1' && data[6] == '.') {
            return 2; // http response
        }
    }
    if ((header_type & 0x80) && data_len >= 5) {
